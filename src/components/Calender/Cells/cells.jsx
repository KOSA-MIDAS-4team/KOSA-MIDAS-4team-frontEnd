...
const Day = (props) => {
  ...
  useEffect(() => {
    async function draw() {
      if (isSameMonth(props.day, props.monthStart)) {
        ...
        const requestDay = `${format(props.day, 'yyyy')}-${format(
          props.day,
          'M',
        )}-${today}`;
        ...
        await axios
          ...
          .then((res) => {
            if (res.data.startedAt !== res.data.endedAt) {
              console.log(
                res.data.startedAt + ' ' + res.data.endedAt + ' ' + requestDay,
              );
            }
  ...
  return (
    ...
    >
      ...
      {workTime.startedAt !== undefined ? (
        <>
          <div>
            {workTime.startedAt !== undefined ? (
              `출근시간 ${workTime.startedAt.substring(11)}`
            ) : (
              <></>
            )}
          </div>

          {workTime.endedAt !== undefined &&
          workTime.startedAt !== workTime.endedAt ? (
            <div>퇴근시간 {workTime.endedAt.substring(11)}</div>
          ) : (
            <></>
          )}
        </>
